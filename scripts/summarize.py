...
def main():
    ...
    if 'scan' in actions or 'all' in actions: # True if actions is either 'all' or ['all']
        aggregate_station_years_by_scan(args.root,
                                        args.stations,
                                        args.years,
                                        args.max_scans,
    ...
    if 'resample' in actions or 'all' in actions:
        resample_station_years(args.root,
                               args.stations,
                               args.years,
                               freq=resample_frequency)

    if 'daily' in actions or 'all' in actions:
        aggregate_station_years_to_daily(args.root,
                                         args.stations,
                                         args.years,
                                         freq=resample_frequency)


def aggregate_station_years_by_scan(root, stations, years, max_scans, chunk_size=100):
            ...
            def read_chunks():
                ...
                for pos in tqdm(range(0, len(profile_paths), chunk_size)):
                    yield util.aggregate_profiles_to_scan_level(
                        profile_paths[ pos : pos + chunk_size ],
                        lat_lon
                    )
