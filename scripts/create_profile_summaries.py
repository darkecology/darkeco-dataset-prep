...
def main(): 
    ...
    for year in years:
        ...
        file_list_paths = [f for f in file_list_paths if '%s' % year in f]
        ...
        for file_list_path in file_list_paths:
            ...
            with open(f'{meta_file_folder}/{station_year}.csv', 'w', newline='') as outfile:
                ...
                outfile.write('station,lat,lon,year,month,day,time,total_reflectivity,avg_u,avg_v,avg_groundspeed,avg_track,total_percent_rain\n')
...
def extract_scan_meta_data(infile, lat_lon):
    ...
    bin_widths = list(set(np.diff(scan['height'])))
    if len(bin_widths) == 1:
    ...
    total_reflectivity = sum(bin_width * linear_eta)

    # Average velocity weighted by reflectivity
    avg_u = sum(scan['u'] * linear_eta) / sum(linear_eta)
    avg_v = sum(scan['v'] * linear_eta) / sum(linear_eta)

    # TODO: double check calculation
    avg_groundspeed = np.sqrt(avg_u ** 2 + avg_v ** 2)
    avg_track = np.arctan2(avg_v, avg_u)

    # TODO: double check calculation
    total_percent_rain = sum(scan['percent_rain'] * scan['nbins']) / sum(scan['nbins'])

    row = f'{station},{lat_lon[station]["lat"]},{lat_lon[station]["lon"]},{year},{month},{day},{time},{total_reflectivity:.4f},{avg_u:.4f},{avg_v:.4f},{avg_groundspeed:.4f},{avg_track:.4f},{total_percent_rain}'

    return row

    # return station, lat_lon[station]['lat'], lat_lon[station]['lon'], year, month, day, time, total_reflectivity, avg_u, avg_v, avg_groundspeed, avg_track, total_percent_rain
